...
st.markdown("""
    ...
    .announcement-container {
        background-color: #fff9db; border: 3px solid #fcc419; border-radius: 20px;
        padding: 30px; margin-bottom: 40px; text-align: center;
        box-shadow: 0 10px 20px rgba(0,0,0,0.1);
    }
    .announcement-title { color: #e67700; font-size: 2em; font-weight: bold; margin-bottom: 20px; }
    ...
    .roulette-text { color: #ff4b4b; font-weight: bold; font-size: 1.2em; animation: blink 0.3s infinite; }
...
if uploaded_file:
    ...
    for g_num, col in zip([1, 2, 3], [c1, c2, c3]):
        ...
        col.markdown(f'<div class="group-container"><h4 style="text-align:center;">Group {g_num} ({len(group_data)}명)</h4>{inner_html}</div>', unsafe_allow_html=True)
    ...
    # [중요] 최상단에 애니메이션 전용 공간 확보
    announcement_placeholder = st.empty()

    col_ctrl1, col_ctrl2 = st.columns([1, 4])
    if col_ctrl1.button("🔥 전체 팀 구성 생성"):
    ...
    if st.session_state.teams_list:
        if st.session_state.current_team_idx < 15:
            if st.button(f"➡️ {st.session_state.current_team_idx + 1}팀 발표 시작"):
                ...
                # 리런 없이 아래 애니메이션 코드로 진입

        # 3. 애니메이션 로직 (상단 노티 박스에서 실행)
        if st.session_state.is_animating:
            idx = st.session_state.current_team_idx
            team_members = st.session_state.teams_list[idx]
            confirmed_html = ""
            ...
            for m_idx, final_m in enumerate(team_members):
                ...
                for _ in range(8):
                    ...
                    roulette_html = f'<div class="member-row" style="background:#eee;"><div class="roulette-text">🎲 추첨 중... {temp["성명"]}</div></div>'
                    announcement_placeholder.markdown(f'<div class="announcement-container"><div class="announcement-title">📢 {idx+1}팀 멤버 발표 중!</div><div style="display:flex; justify-content:center; gap:20px;">{confirmed_html}{roulette_html}</div></div>', unsafe_allow_html=True)
                    time.sleep(0.06)
                
                # 확정된 멤버 추가
                is_g1 = "g1-bg" if float(final_m['그룹']) == 1.0 else ""
                badge = '<span class="badge">G1</span>' if float(final_m['그룹']) == 1.0 else ""
                confirmed_html += f'<div class="member-row {is_g1}" style="width:250px; background:white; border:1px solid #ddd;"><div><span class="member-name">{final_m["성명"]}</span>{badge}</div><div style="font-size:0.8em; color:gray;">{int(final_m["학년"])}학년 | {final_m["학과"]}</div></div>'
                announcement_placeholder.markdown(f'<div class="announcement-container"><div class="announcement-title">📢 {idx+1}팀 멤버 발표 중!</div><div style="display:flex; justify-content:center; gap:20px;">{confirmed_html}</div></div>', unsafe_allow_html=True)
                time.sleep(0.4)
            
            time.sleep(1) # 발표 완료 후 잠시 대기
            st.session_state.current_team_idx += 1
            st.session_state.is_animating = False
            announcement_placeholder.empty() # 알림창 닫기
            st.rerun() # 전체 리스트 업데이트를 위해 리런
        ...
        if st.session_state.current_team_idx > 0:
            ...
            for visual_idx, real_idx in enumerate(display_indices):
                ...
                t_cols[visual_idx % 3].markdown(f'<div class="team-card"><div class="team-title">TEAM {real_idx+1}</div>{m_html}</div>', unsafe_allow_html=True)
